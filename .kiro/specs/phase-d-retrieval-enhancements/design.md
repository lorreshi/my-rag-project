...
### 四个易混概念的分工（检索 / 过滤 / 抽取 / 同义词）

这四者常被混淆，但职责完全不同。一句话区分：**检索负责"软"召回排序，过滤负责"硬"属性约束，抽取负责把约束从文本里解析出来，同义词负责扩大关键词召回**。

| 概念 | 作用对象 | 输入 → 产出 | 目的 | 比喻 |
|------|---------|------------|------|------|
| **检索** dense/sparse | query 向量 / 关键词 ↔ chunk 向量 / BM25 词表 | query → 一批"相关"候选（带分数排名） | 召回 + 软排序 | 搜索引擎给你一长串相关结果 |
| **过滤** `match_filters` | chunk 的**元数据标签**（doc_type/sheet_name/is_table…） ↔ `filters` 字典 | 候选 + filters → 剔除属性不符的候选 | 硬性精确约束 | 左侧"只看PDF/只看2024"的勾选框 |
| **抽取** `filter_extractor` | query**文本** | query → 结构化**约束** `{sheet_name: …}` | 把过滤条件从自然语言/语法里解析出来 | 把"在Q1表里"翻译成机器能用的过滤项 |
| **同义词** `expanded_keywords` | 已分词的**关键词** | 关键词 → 更多关键词 `[接口, api]` | 扩大 BM25 召回 | 搜"接口"时顺带也搜"api" |

关键边界澄清（对应实际易错点）：
- **过滤 ≠ 检索**：过滤只看元数据标签做等值匹配，**完全不碰向量与关键词**；向量/关键词是上一步检索用的。
- **抽取 ≠ 同义词**：抽取产出的是**过滤约束**（缩小范围），同义词产出的是**更多关键词**（扩大召回），两条线相互独立、互不影响。
- **抽取 → 过滤**：抽取是"约束从哪来"（默认关，仅 `RuleBasedFilterExtractor` 认 `key:value`），过滤是"约束怎么用"（始终在跑）。
- **同义词 → 检索**：同义词只增强 sparse（BM25）这一路，稠密侧保持单次 embedding。

```mermaid
flowchart LR
    Q["原始查询"] --> A["抽取 filter_extractor<br/>文本→结构化约束<br/>(默认关)"]
    Q --> S["同义词扩展<br/>关键词→更多关键词<br/>(默认关，仅BM25)"]
    Q --> R["检索 dense/sparse<br/>软召回+排序"]

    A -->|"filters{}"| F["过滤 match_filters<br/>按元数据硬约束<br/>(始终在跑)"]
    S -->|"expanded_keywords"| R
    R -->|"候选(带分数)"| F
    F --> TOPK["合规且相关的候选 → 重排 → Top-K"]

    subgraph 缩小范围["缩小范围（精准）"]
        A
        F
    end
    subgraph 扩大召回["扩大召回（找得全）"]
        S
        R
    end
```
...
```mermaid
flowchart TD
    Q["query"] --> QP["QueryProcessor.process()<br/>_normalize 仅折叠空白<br/>filters 仅透传外部"]
    QP -->|"normalized_query"| CK["candidate_k = top_k × 2<br/>（硬编码，忽略 settings）"]
    QP -->|"keywords"| CK
    CK --> D["DenseRetriever.retrieve<br/>(normalized, filters)"]
    CK --> S["SparseRetriever.retrieve(keywords)<br/>❌ 无 filters，绕过前置过滤"]
    D --> F["ReciprocalRankFusion.fuse()<br/>❌ 硬编码 RRF，忽略 fusion_algorithm"]
    S --> F
    F --> PF["_apply_metadata_filters()<br/>后置兜底（唯一的稀疏过滤点）"]
    PF --> TK["Top-K"]
    TK --> RR["Reranker.rerank()<br/>❌ head=cross-encoder 分 / tail=RRF 分<br/>尺度混用、非单调"]
    RR --> OUT["结果"]

    classDef bug fill:#fde,stroke:#c66;
    class S,F,RR bug;
```
...
```mermaid
flowchart TD
    Q["query"] --> QT["§8/§9 QueryTransform.transform()<br/>none恒等 / multi_query N变体 / hyde假设文档<br/>LLM失败降级单查询（默认关）"]
    QT --> QP["QueryProcessor.process()"]

    subgraph PROC["查询处理"]
        direction TB
        NM["§1 _normalize: 折叠空白 + NFKC/繁简（稠密侧）"]
        KW["keywords: 经共享分词层（NFKC 统一 → BM25 词表对齐）"]
        EK["§7 expanded_keywords: keywords + 同义词/别名（OR，默认关）"]
        FL["§2 filters: 外部 ⊕ 规则抽取的结构化约束（默认关）"]
    end
    QP --> PROC

    PROC --> CK["§3 候选池宽度（配置驱动）<br/>dense_k = top_k_dense × multiplier<br/>sparse_k = top_k_sparse × multiplier"]

    CK --> D["DenseRetriever.retrieve × (1 或 N 条)<br/>取决于 QueryTransform"]
    CK --> S["§5 SparseRetriever.retrieve(expanded_keywords, filters)<br/>✅ 前置过滤：over-fetch + 解析期过滤"]

    D --> F["§4 FusionFactory.fuse([dense_1..N, sparse], weights)<br/>✅ rrf(可加权) | weighted_sum"]
    S --> F
    F --> PF["§2 _apply_metadata_filters() 后置兜底<br/>strict/lenient 策略不变"]
    PF --> TK["Top-K"]
    TK --> RR["§6 Reranker.rerank()<br/>✅ head/tail 统一到单调一致尺度<br/>原始分存 metadata"]
    RR --> MMR["§10 MMR 多样性<br/>（可选，默认关；λ=1 恒等）"]
    MMR --> TH["§11 阈值闸门<br/>（可选，默认关；Top1<阈值→返回空 abstain）"]
    TH --> OUT["结果"]

    classDef new fill:#dfe,stroke:#6a6;
    classDef opt fill:#eef,stroke:#88c;
    class S,F,RR new;
    class QT,EK,FL,MMR,TH opt;
```

> 图例：绿色=本期修复/增强且默认生效（归一化、配置接线、前置过滤、加权融合、分数统一）；蓝色=可插拔且**默认关**（查询变换、同义词、filters 抽取、MMR、阈值）——默认配置下整条链路与 Before 逐项一致（Property 9）。
...
### filters 的两层关系（抽取 vs 过滤）

务必区分两件事，它们常被混淆：

- **「抽取」**：filters 从哪来。当前仅一个 `RuleBasedFilterExtractor`，只认显式 `key:value`（自然语言抽不出），覆盖 7 键白名单，**默认关**。这是当前较弱、留待后续（规则增强 / 已知表名匹配 / LLM 抽取器）补强的一环——接口 `BaseFilterExtractor` 已为无痛替换预留。
- **「过滤」**：filters 怎么用。无论来源是调用方显式传入还是抽取器产出，只要 filters 非空，`match_filters` 的 strict/lenient 策略**始终生效**（dense 后置 + sparse 前置）。这一层骨架完整、默认在跑。

```mermaid
flowchart TD
    Q["原始查询<br/>'sheet_name:Q1营收 毛利率'"] --> QP["QueryProcessor.process(query, filters=外部?)"]

    subgraph L1["第一层：抽取（filters 从哪来）"]
        direction TB
        EXT{"filter_extractor<br/>是否注入?"}
        EXT -->|"None（默认）"| NOEXT["不抽取 → {}"]
        EXT -->|"RuleBasedFilterExtractor<br/>（唯一实现，默认关）"| RULE["只认 key:value<br/>白名单7键<br/>自然语言→抽不到"]
        RULE -.未来可换.-> FUT["SheetName/LLM 抽取器<br/>（接口已预留，尚未实现）"]
    end

    QP --> EXT
    NOEXT --> MERGE
    RULE --> MERGE["_parse_filters 合并<br/>外部 filters 优先覆盖抽取"]
    MERGE --> PF["ProcessedQuery.filters"]

    subgraph L2["第二层：过滤（filters 怎么用，始终在跑）"]
        direction TB
        MF["match_filters<br/>strict: 结构化键缺失→排除<br/>lenient: 通用键缺失→保留"]
        MF --> DENSE["dense 后置过滤"]
        MF --> SPARSE["sparse 前置过滤（over-fetch）"]
    end
...
核心抽象：三种模式都产出一个 `dense_queries` 文本列表，每条文本各做一次稠密检索得到一条排名列表，所有稠密列表 + 单条稀疏列表交给既有 `Fusion.fuse(list[list])`——**融合层零改动**。`none`（默认）产出 `[query]` 单条列表，行为与基线逐字一致。

```mermaid
flowchart TD
    Q["原始 query"] --> T{"retrieval.query_transform<br/>选择模式"}

    T -->|"none（默认）"| N["NoOpTransform<br/>→ [query]"]

    T -->|"multi_query"| M["MultiQueryTransform<br/>LLM 改写 N 个等价问法<br/>原 query 始终在列"]
    M --> MC{"缓存命中?"}
    MC -->|是| MV["复用变体（不调 LLM）"]
    MC -->|否| ML["调 LLM 改写 → 写缓存"]
    M -.->|"LLM 失败"| DG["降级 [query]<br/>degraded=True"]

    T -->|"hyde"| H{"目标 doc_type<br/>∈ skip_doc_types?"}
    H -->|"是（如 xlsx）"| HS["跳过 → [query]"]
    H -->|否| HG["LLM 生成假设文档 hypo"]
    HG --> HA{"augment?"}
    HA -->|"True（默认）"| HB["[query, hypo]"]
    HA -->|False| HC["[hypo]"]
    H -.->|"LLM 失败"| DG

    N --> DQ["dense_queries 文本列表"]
    MV --> DQ
    ML --> DQ
    DG --> DQ
    HS --> DQ
    ...
    DQ --> RUN["对每条文本各跑一次稠密检索<br/>（并发上限约束 embedding 调用）"]
    RUN --> DL["N 条稠密列表"]
    SP["稀疏列表（单条，不受变换影响）"] --> FUSE
    DL --> FUSE["Fusion.fuse([*dense_lists, sparse])<br/>★ 融合层零改动"]
    FUSE --> NEXT["→ 过滤 → rerank → MMR → Top-K"]
```

> 关键点：HyDE 生成的假设文档**事实可能是错的，但只用于检索探路**，最终返回的始终是检索回的真实 chunk。`multi_query` 与 HyDE 都只增强稠密侧；稀疏侧（BM25）始终单条列表，由 §7 的同义词扩展独立负责。
...
纯相关性排序有个副作用：**最相关的几条往往彼此也很像**，Top-K 被「近重复」挤满——信息量虚高，真正多样且有用的内容反被挤出。本项目两个典型场景：**同名干扰**（搜「刘静的绩效」，Top-5 几乎都是刘静的雷同片段，把「周静」挤掉）与**同表相似行**（一张表的相似行分数都高，占满 Top-K）。

MMR（Maximal Marginal Relevance）在「与 query 相关」之外，额外加一项**对已选结果的冗余惩罚**：每选下一条都要求「既相关、又与已选的不一样」。`λ` 调两者比重——`λ=1` 退化为纯相关性（等价不开 MMR），`λ=0.5`（默认）相关性/多样性各半。贪心选择：一旦某候选与已选太像，惩罚项把它压低，逼算法转去选「也相关但角度不同」的内容。

```
纯相关性 Top-3:  [A刘静, B刘静, C刘静]    ← 全是刘静，周静被挤掉
MMR(λ=0.5):     [A刘静, D周静, B刘静]    ← 选完A后B/C因与A太像被压，D冒头
```

放在 rerank 之后、最终裁剪之前；复用检索阶段已有的稠密向量，缺向量则降级原序、绝不报错；默认关（补算向量有成本）。
